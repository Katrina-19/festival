...
function selectAllFromPostsWithUser($table1, $table2,$id){
    global $pdo;
    $sql="
    SELECT 
    t1.id, 
...
function selectTopFromPostsOnIndex($table1){
    global $pdo;
    $sql="
    SELECT * FROM $table1 WHERE id_topic=24";
    $query =$pdo->prepare($sql);
    $query->execute();
    dbCheckError($query);
    return $query->fetchAll();
...
function search($text,$table1, $table2){
    ...
    global $pdo;
    $sql="
    SELECT p.*,u.username FROM $table1 AS p JOIN $table2 AS u ON p.id_user=u.id WHERE p.status=1 AND p.title LIKE '%$text%' OR p.content LIKE '%$text%'";
    $query =$pdo->prepare($sql);
    $query->execute();
    dbCheckError($query);
    return $query->fetchAll();
...
function selectPostFromPostsWithUsersOnIndex($table1, $table2, $id){
    global $pdo;
    $sql="
